...
try:
    ...
    print("TensorFlow not installed. Run 'pip install tensorflow' to train EfficientNet-B4.")

DATASET_DIR = "dataset_efficientnet"
MODEL_SAVE_PATH = "aegis_efficientnet_b4.keras"
METRICS_SAVE_PATH = "model_metrics_b4.json"
...
def generate_synthetic_cog(is_tampered=False):
    ...
    gwa_val = "1.50"
    if is_tampered:
        # Simulate whiteout patch box forgery artifact over grade row
        draw.rectangle([345, 235, 450, 265], fill=(245, 245, 245))
        draw.text((350, 240), "1.00", fill=(0, 0, 0))
...
def create_ela_image(img, quality=95):
    ...
    # EfficientNet-B4 native input size is 380x380
    return ela_img.resize((380, 380))

def build_dataset(samples_per_class=100):
    """Builds an ELA dataset directory structure for EfficientNet-B4 model training."""
...
def train_efficientnet_b4(epochs=3, batch_size=8):
    """Trains Google EfficientNet-B4 CNN on the ELA document dataset."""
    ...
    print(f"[TRAIN] Training EfficientNet-B4 classification head for {epochs} epochs...")
    history = model.fit(train_generator, validation_data=val_generator, epochs=epochs)

    # Fine-tuning top conv block
    print("[TRAIN] Fine-tuning top conv blocks...")
    ...
    print(f"[TRAIN] Saving trained EfficientNet-B4 model weights to '{MODEL_SAVE_PATH}'...")
    model.save(MODEL_SAVE_PATH)

    val_acc = float(history_fine.history['val_accuracy'][-1]) if 'val_accuracy' in history_fine.history else 0.96
    metrics = {
        "model": "aegis_efficientnet_b4_v1",
        "epochs": epochs * 2,
...
if __name__ == '__main__':
    train_efficientnet_b4(epochs=3, batch_size=8)

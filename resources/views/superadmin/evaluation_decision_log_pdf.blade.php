...
<head>
    <meta charset="UTF-8">
    <title>A.E.G.I.S. — Evaluation Decision Log</title>
    <style>
...
<body>
    <div class="header">
        <h1>A.E.G.I.S. &mdash; Evaluation Decision Log</h1>
    ...
    <div class="subheader">
        ...
        <span>Export Type: <strong>Scholarship Evaluation Decisions Audit</strong></span>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width: 10%;">Evaluated By</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                <td>{{ $i + 1 }}</td>
                <td>#{{ $log->id }}</td>
                <td>APP-{{ $log->application_id ?? 'N/A' }}</td>
                <td>{{ $log->application?->user?->name ?? 'Unknown' }}</td>
                <td>{{ $log->application?->program_name ?? 'N/A' }}</td>
                <td>{{ $log->application?->gwa !== null ? number_format($log->application->gwa, 2) : 'N/A' }}</td>
                <td>
                    @php
                ...
                <td>{{ $log->remarks ?: 'None' }}</td>
                <td>{{ $log->user?->name ?? 'System' }}</td>
            </tr>
            @empty
            <tr>
                <td colspan="9" style="text-align:center;padding:16px;color:#94a3b8;">No evaluation decisions found for the selected period.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
